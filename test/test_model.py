...
class TestModel(unittest.TestCase):
    ...
    def test_mnist_run_model(self):
        self.run_model_common(
            "MNIST", config.dataset_file_path, "ModelCNNMnist", 10)
        self.run_model_common("MNIST", config.dataset_file_path, "LeNet5", 10)

    def test_svhn_run_model(self):
        self.run_model_common(
            "SVHN", config.dataset_file_path, "WResNet40-2", 10)
        self.run_model_common(
            "SVHN", config.dataset_file_path, "WResNet16-1", 10)

    def test_cifar_10_run_model(self):
        self.run_model_common(
            "cifar10", config.dataset_file_path, "ModelCNNCifar10", 10)
        self.run_model_common(
            "cifar10", config.dataset_file_path, "ResNet34", 10)
        self.run_model_common(
            "cifar10", config.dataset_file_path, "ResNet18", 10)

    def test_cifar_100_run_model(self):
        self.run_model_common(
            "cifar100", config.dataset_file_path, "ResNet34", 100)
        self.run_model_common(
            "cifar100", config.dataset_file_path, "ResNet18", 100)
