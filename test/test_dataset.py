...
class TestDataset(unittest.TestCase):
    ...
    def test_mnist_load_data(self):
        self.load_data_common("MNIST", config.dataset_file_path, "ModelCNNMnist", 60000, 10000, 28, 28, 1)
        self.load_data_common("MNIST", config.dataset_file_path, "LeNet5", 60000, 10000, 32, 32, 1)

    def test_svhn_load_data(self):
        self.load_data_common("SVHN", config.dataset_file_path, "WResNet40-2", 73257, 26032, 32, 32, 3)

    def test_cifar_10_load_data(self):
        self.load_data_common("cifar10", config.dataset_file_path, "ModelCNNCifar10", 50000, 10000, 32, 32, 3)

    def test_cifar_100_load_data(self):
        self.load_data_common("cifar100", config.dataset_file_path, "ResNet34", 50000, 10000, 32, 32, 3)

    def test_femnist_load_data(self):
        self.load_data_common("FEMNIST", config.dataset_file_path, "ModelCNNEmnist", -1, -1, 28, 28, 1)

    def test_celeba_load_data(self):
        self.load_data_common("celeba", config.dataset_file_path, "ModelCNNCeleba", -1, -1, 84, 84, 3)
